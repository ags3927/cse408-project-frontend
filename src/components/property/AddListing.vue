<template>
	<div class="container">
		...
		<div class="card shadow my-card-border mt-3 mx-auto" style="width: 60%">
			...
			<div id="step-05" v-if="step===5" class="text-left mx-4 px-3" updateMarker>
				...
				<gmap-map
					...
					<gmap-marker
						...
						@click="center=propertyObject.location.coordinates">
					</gmap-marker>
				</gmap-map>
			</div>
					...
					<div class="row"
						...
						<div class="col-12 mx-auto">
							<img
								...
								src="../../assets/img/properties/bg-image.png" alt="Background Photo"
...
export default {
	...
	data() {
		return {
			...
			imageUploaded: false,
			imageUploadBG: '../../assets/img/properties/bg-image.png',
			uploadedImage: null,
			guestRequirements: {
				mandatory: [
					'Email address',
	...
	methods: {
		...
		validateStep() {
			if (this.step === 1) {
				if (this.propertyObject.typeOfProperty === null || this.propertyObject.guestAccess === '') {
		...
		uploadImage() {
			...
			let fd = new FormData();
			fd.append('upload_preset', 'skbeqpnf');
			fd.append('folder', 'property-images');
		...
		finish() {
			...
			axios.post(baseURL + 'property/insert', {propertyObject: this.propertyObject}, {
				headers: {
					'x-auth': sessionToken
				}
			}).then(res => {
